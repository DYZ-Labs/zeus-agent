...
export type GoogleCalendarIntegrationConfiguration =
  | { state: "unconfigured" }
  | { state: "misconfigured"; message: string }
  | { state: "ready"; brokerUrl: string; serviceKey: string };

export function getGoogleCalendarIntegrationConfiguration(
  environment: Readonly<Record<string, string | undefined>> = process.env,
): GoogleCalendarIntegrationConfiguration {
  const brokerUrl = environment.GOOGLE_CALENDAR_BROKER_URL?.trim() ?? "";
  ...
  try {
    ...
    return {
      state: "misconfigured",
      message:
        "Google Calendar needs an HTTPS broker URL and a service key of at least 32 bytes.",
    };
  }
}
