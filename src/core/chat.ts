...
import { markCommitmentSurfaced } from "./intentions";
import type { Message, SearchHit } from "./schema";

/** Stable cached prefix. All volatile memory remains in the final user turn. */
const SYSTEM_PROMPT = `You are Zeus, a personal copilot with a durable memory of one person: the user you are talking to.
...
Answer personally when the accepted memory helps. Do not announce that you consulted memory or dump everything recalled. If a goal or commitment lacks a detail that materially affects follow-through, ask at most one focused clarification.

The block may contain one OPTIONAL NUDGE CANDIDATE. First answer the user's request. Then, only when the item is relevant or overdue, add at most one short, non-repetitive nudge. Never claim to have performed an external action and never act without explicit approval.
...
export async function streamTurn(db: Db, options: StreamTurnOptions): Promise<TurnResult> {
  ...
  if (context.nudge) markCommitmentSurfaced(db, context.nudge.id);
