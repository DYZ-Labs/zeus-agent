...
/**
 * The slot that sends, and the first time this CHECK has admitted `send`.
 *
 * Every previous rebuild of this table left `send` out of `effect_kind`, and a test asserted
 * that leaving it out was load-bearing. It was: email had no send until the user asked for
 * one. Widening it is a deliberate decision, not a tidy-up, so the pairing CHECK stays exactly
 * as strict — `send` pairs with `email.send_message` and with nothing else, and
 * `email.send_message` pairs with nothing but `send`. A row claiming the calendar can send, or
 * that sending is a read, still cannot be written.
 *
 * What the database can no longer do on its own is stop a send. That job moves to the places
 * named in `schema.ts`: a code-owned builder, no standing policy, a recipient allowlist
 * checked twice, and one exact confirmation.
 */
const EMAIL_SEND_CAPABILITY_SLOT = `
ALTER TABLE connector_capability RENAME TO connector_capability_send_legacy;
CREATE TABLE connector_capability (
  id                 INTEGER PRIMARY KEY,
  connector_id       INTEGER NOT NULL REFERENCES connector(id) ON DELETE CASCADE,
  slot               TEXT NOT NULL
  ...
  remote_tool_name   TEXT NOT NULL,
  effect_kind        TEXT NOT NULL
                     CHECK (effect_kind IN ('external_read','schedule','modify_external','send')),
  input_schema_json  TEXT NOT NULL CHECK (json_valid(input_schema_json)),
  schema_hash        TEXT NOT NULL
                     CHECK (length(schema_hash) = 64 AND schema_hash NOT GLOB '*[^0-9a-f]*'),
  enabled            INTEGER NOT NULL DEFAULT 0 CHECK (enabled IN (0,1)),
  ...
  CHECK (
    ...
    OR (slot = 'email.trash_thread' AND effect_kind = 'modify_external')
    OR (slot = 'email.untrash_thread' AND effect_kind = 'modify_external')
    OR (slot = 'email.send_message' AND effect_kind = 'send')
  )
);
INSERT INTO connector_capability
  (id, connector_id, slot, remote_tool_name, effect_kind, input_schema_json, schema_hash,
...
SELECT id, connector_id, slot, remote_tool_name, effect_kind, input_schema_json, schema_hash,
       enabled, source_message_id, created_at, updated_at
FROM connector_capability_send_legacy;
DROP TABLE connector_capability_send_legacy;
CREATE INDEX connector_capability_slot_idx ON connector_capability(slot, enabled);
`;
...
export const MIGRATIONS: readonly Migration[] = [
  ...
  { id: "040_email_send_capability_slot", sql: EMAIL_SEND_CAPABILITY_SLOT },
];
