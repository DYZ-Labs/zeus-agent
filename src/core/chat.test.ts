...
const mocks = vi.hoisted(() => ({
  ...
  markRecommendationSurfaced: vi.fn(),
  on: vi.fn(),
  openaiStream: vi.fn(),
...
vi.mock("./stewardship", () => ({
  markRecommendationSurfaced: mocks.markRecommendationSurfaced,
}));
...
describe("streamTurn cancellation", () => {
  it("passes the signal to OpenAI and does not persist or extract an aborted reply", async () => {
    ...
    expect(mocks.markRecommendationSurfaced).not.toHaveBeenCalled();
