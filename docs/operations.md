...
How to run, configure, host, back up, and recover a Zeus installation. The product it
serves is described in [README.md](../README.md); everything here is operational.

These sections were part of README.md until commit `531e527` rewrote that file as a
product document. They are restored here unchanged in substance, with the stale parts
...
| `ZEUS_SIGNAL_REFRESH` | No | on | In-process detector refresh (`on`/`off`); `off` leaves refresh to the macOS ambient worker |
...
{"status":"ok","store":{"status":"ok","migrations_applied":30,"migrations_expected":30},
 "auth":"local","embeddings":"ready","model":"configured"}
```

The migration counts move with every schema change, so the number is only ever a sample.
What matters is that the two agree: applied behind expected means this process has not
finished migrating, and applied *ahead* of expected means the deploy went backwards.
...
The server provides these memory tools:
...
- `zeus_follow_through_feedback` — record an explicitly authorized decision on that
  recommendation.
- `zeus_behavioral_policy_suggestions` — list non-canonical behavioral-policy suggestions.
- `zeus_review_behavioral_policy_suggestion` — accept or reject one of them explicitly.

And these bounded-work and effect tools, which carry the same two gates the web interface
does — authorizing a plan approves a scope, confirming an effect approves a payload:

- `zeus_create_work_plan` — create a bounded local work-plan proposal.
- `zeus_authorize_work_plan` — authorize one exact plan.
- `zeus_run_work_plan` — run one exactly authorized plan.
- `zeus_resume_work_run` — resume one exact run.
- `zeus_work_plan_status` — read a plan, its run, and its artifacts.
- `zeus_cancel_work_plan` — cancel a plan.
...
the user's Calendar grant. In the Gmail project, enable `gmail.googleapis.com` (the broker calls the Gmail REST API
directly; `gmailmcp.googleapis.com` is a gated preview Zeus no longer uses), register
