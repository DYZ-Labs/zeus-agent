...
const databasePath = resolve(option("--db") ?? defaultDbPath());
const directory = resolve(option("--directory") ?? defaultSnapshotDirectory(databasePath));
const retention = option("--retention")
  ? parseRetention(option("--retention")!)
  : configuredSnapshotRetention();

assertExistingRegularDatabase(databasePath);
...
try {
  ...
  process.stdout.write(`Created verified snapshot ${result.path}; pruned ${result.pruned}.\n`);
} finally {
  db.close();
}
